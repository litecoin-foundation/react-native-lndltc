...
class ReactNativeLightning: NSObject {
  private var storage: URL {
    ...
    let directory = documentsDirectory.appendingPathComponent("lndltc")
  ...
  lazy var syncMethods: [String: (Data?, BlindLndCallback) -> Void] = {
    return [
      ...
      "SetScores": { (req: Data?, cb: BlindLndCallback) in LndmobileAutopilotSetScores(req, cb) },
      "Status": { (req: Data?, cb: BlindLndCallback) in LndmobileAutopilotStatus(req, cb) },
      "QueryScores": { (req: Data?, cb: BlindLndCallback) in LndmobileAutopilotQueryScores(req, cb) },
      ...
      "ModifyStatus": { (req: Data?, cb: BlindLndCallback) in LndmobileAutopilotModifyStatus(req, cb) },
      ...
      "AddTower": { (req: Data?, cb: BlindLndCallback) in LndmobileWatchtowerClientAddTower(req, cb) },
      "GetTowerInfo": { (req: Data?, cb: BlindLndCallback) in LndmobileWatchtowerClientGetTowerInfo(req, cb) },
      "ListTowers": { (req: Data?, cb: BlindLndCallback) in LndmobileWatchtowerClientListTowers(req, cb) },
      "RemoveTower": { (req: Data?, cb: BlindLndCallback) in LndmobileWatchtowerClientRemoveTower(req, cb) },
      "Stats": { (req: Data?, cb: BlindLndCallback) in LndmobileWatchtowerClientStats(req, cb) },
    ]
  }()
...
//      "SendToRoute": { (req: Data?, cb: BlindLndCallback) in LndmobileSendToRoute(req, cb) }, TODO these probably need to be passed pointers to a LndmobileSendStream
  ...
  func walletExists(_ network: NSString, resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock) {
    resolve(FileManager.default.fileExists(atPath: storage.appendingPathComponent("/data/chain/litecoin/\(network)/wallet.db").path))
  }
...
extension ReactNativeLightning {
    func getLogFile(_ network: NSString) -> String {
        return self.storage.appendingPathComponent("logs/litecoin/\(network)/lnd.log").path;
    }
